...
func (u *taskUseCase) GetTasksByCompanyId(companyId uint, limit, offset int) ([]*model.Task, error) {
	_, err := u.companyRepository.GetCompany(companyId)
	if err != nil {
		return nil, err
	}

	tasks, err := u.taskRepository.GetTasksByCompanyId(companyId, limit, offset)
	if err != nil {
		return nil, err
	}
	return tasks, nil
